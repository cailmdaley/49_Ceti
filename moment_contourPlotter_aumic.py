...
#.fits inputs
dat = ['../AU_Mic/Visibilities/aumic_ALL.fits']
...
def plotcmd(dat,ax,levs1):
    ...
    if i == 0:
        axT.set_xticks(np.arange(-0.0001, 0.000300001,0.0001))
        axT.set_xticklabels(range(-100,301,100),fontsize=7,rotation=30)
    ...
    if ax.is_first_col():
        ...
        ax.text(115,383,'AU Mic',fontsize=8,path_effects=[PathEffects.withStroke(linewidth=2,foreground="w")])#color='k')
        ax.text(115,370,r'ALMA 1350$\mu m$',fontsize=8,path_effects=[PathEffects.withStroke(linewidth=2,foreground="w")])
        ax.text(50,873,'Moment 0',fontsize=8,path_effects=[PathEffects.withStroke(linewidth=2,foreground="w")])
        ax.text(236,405,r'$\mu Jy / bm$',fontsize=8,path_effects=[PathEffects.withStroke(linewidth=2,foreground="w")])


for i in range(1):
	if i == 0:
		levels = np.array([j*3.6174e-5 for j in range(2,40,2)])
...
plt.savefig("/home/cdaley/Documents/AU_Mic/Visibilities/aumic_ALL.png",dpi=400)
plt.show(False)
